...
int main(){
	...
	while(option != 5)
	{
		...
		cout << "3. Sort cars by color" << endl;
...
void insert_linkedList(List *list){
	...
	int count = 1; // begin with element 1
	struct Car *cars;
	while(infile >> x)
	{
		switch(count) {
			case 1 : 
				// initislize cars when a new row is reached
				cars = new Car;
				cars->make = x;
				count++;
				break;
			...
			case 4 : 
				cars->color = x;
				// once the structure has read in its row, insert the cars into the list
...
void sort_cars_by_color(List *list){
	...
	{
		// ensures current is located at appopriate position in the list
		ListHead(list);
		if(i>0){
			for(int k=1; k<=i; k++){
				ListNext(list);
		...
		// saves first, its previous, its next, and its value for color before moving on to second
		first = list->current;
		firstprev = list->previous;
		temp = first->next;
		string min = list->current->color;
		second = list->current;
		// saves second whenever "lower" string is found
...
//void print_duplicates(List *list);
void print_duplicates(List *list){
	// sets to head so that entire list can be searched
	ListHead(list);	
	// sorts list to simplify evaluating equal
    	sort_cars_by_color(list);
	for(int i=0; i<(list->count-2); i++){
		// first checks for equal colors
		if(list->current->colors == list->current->next->colors){
			// then checks that all other members are equal
		...
		// moves current to next element in the list so that next two elements can be evaluated
		ListNext(list);
	}	
}
